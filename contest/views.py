...
def get_problem_list(request, pk):
    ...
    return render(request, 'contest/contest_problem_list.html', {
        ...
        'user_problem_analysis': [
            get_user_contest_problem_analysis(user=request.user, problem=get_object_or_None(Problem, pk=x.problem)) for
            x in contest_problem_set] if request.user.is_authenticated else None,
        'contest': contest, })
...
def get_contest_rank(request, pk):
    ...
    pos_hashtable = {x.problem: i for i, x in enumerate(contest.contestproblem_set.all())}
    sub_all = Submission.objects.raw(
        'SELECT submission_id, judge_status, submit_time, problem_id , user_id FROM submission_submission WHERE contest_id = %d ORDER BY submission_id' % (
            pk))
