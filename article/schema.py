...
class Query(object):
    ...
    articleDiscussionList = graphene.Field( ArticleDiscussionListType , slug = graphene.String() , page = graphene.Int() , time = graphene.Int( required = False ) )
    ...
    def resolve_articleDiscussionList( self , info , slug , page , * args , ** kwargs ):
        from django.core.paginator import Paginator
        discussionList = ArticleDiscussion.objects.filter( article = Article.objects.get( slug = slug ) , reply = None ).order_by( 'submit_time' )
        if not info.context.user.has_perm('article.view_all'):
